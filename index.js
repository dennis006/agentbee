            ...
            case 'crosshair_create':
                await interaction.reply({
                    embeds: [{
                        ...
                        description: '**Erstelle dein perfektes Valorant Crosshair!**\n\nÖffne den Web-Creator für die beste Crosshair-Erstellung:',
                        color: 0x00D4AA,
                        fields: [
                            {
                                name: '🎨 Vollständiger Editor',
                                value: '• Echtzeit Live-Vorschau\n• Custom Color System\n• VCRDB-kompatible Codes\n• Pro & Fun Random Generator',
                                inline: true
                            },
                            {
                                name: '⚡ Einfach zu nutzen',
                                value: '• Intuitive Bedienung\n• Sofortiges Feedback\n• Code kopieren & einfügen\n• Direkter Valorant Import',
                                inline: true
                            }
                        ],
                        thumbnail: {
                        ...
                        footer: {
                            text: 'AgentBee Crosshair Creator • Web-basierte Lösung'
                        },
                    ...
                    components: [{
                        ...
                        components: [{
                            type: 2, // Button
                            style: 5, // Link
                            label: '🎯 Web Creator öffnen',
                            url: 'https://agentbee-dashboard.netlify.app/crosshair',
                            emoji: { name: '🎯' }
                        }]
                    }],
                    ephemeral: true
...
const twitchCommands = {
    hello: {
        response: 'Hey {user}! 👋 Willkommen im Chat!',
        description: 'Begrüßung',
        cooldown: 5000
    },
    commands: {
        response: 'Verfügbare Commands: !hello, !uptime, !game, !followage',
        description: 'Zeigt alle Commands',
        cooldown: 10000
    },
    uptime: {
        response: 'Stream läuft seit: {uptime}',
        description: 'Stream Uptime',
        cooldown: 5000
    },
    game: {
        response: 'Aktuelles Spiel: {game}',
        description: 'Aktuelles Spiel',
        cooldown: 5000
    }
};
...
// Twitch Bot Initialisierung
async function initializeTwitchBot() {
    try {
        if (!apiKeys.twitch.clientId || !apiKeys.twitch.clientSecret) {
            console.log('⚠️ Twitch API Keys nicht gefunden - Twitch Bot deaktiviert');
            return;
        }

        // Lade Twitch Channel Liste aus Supabase
        await loadTwitchChannelsFromSupabase();
        ...
        const channelNames = twitchChannels.map(ch => ch.channelName);
        ...
        twitchBot = new tmi.Client({
            ...
            identity: {
                username: 'AgentBeeBot',
                password: `oauth:${apiKeys.twitch.clientSecret}`
            },
            channels: channelNames
        });

        // Twitch Bot Event Listeners
        twitchBot.on('connected', (addr, port) => {
            ...
            // Speichere Status in Supabase
            saveTwitchBotStatusToSupabase();
        });
        ...
        twitchBot.on('message', (channel, tags, message, self) => {
            ...
            // Update Statistiken
            twitchBotStatus.totalMessages++;
            twitchBotStatus.lastActivity = new Date();

            // Chat Logging
            logTwitchMessage(channel, tags, message);

            // Moderation
            if (shouldModerateMessage(message, tags)) {
                handleTwitchModeration(channel, tags, message);
                return;
            }

            // Command Handling
            if (message.startsWith(twitchSettings.prefix)) {
                handleTwitchCommand(channel, tags, message);
            }

            // Auto Responses
            if (twitchSettings.autoResponses) {
                handleAutoResponses(channel, tags, message);
            }
        });
...
// Lade Twitch Channels aus Supabase
async function loadTwitchChannelsFromSupabase() {
    try {
        if (!supabase) {
            console.log('⚠️ Supabase nicht verfügbar - verwende lokale Twitch-Konfiguration');
            return;
        }
        ...
        twitchChannels = data || [];
        console.log(`📺 ${twitchChannels.length} aktive Twitch Channels geladen`);

    } catch (error) {
        console.error('❌ Fehler beim Laden der Twitch Channels:', error);
        twitchChannels = [];
    }
}

// Twitch Message Logging
async function logTwitchMessage(channel, tags, message) {
    try {
        if (!supabase) return;
        ...
        const logData = {
            ...
            badges: tags.badges,
            emotes: tags.emotes,
            user_type: tags['user-type'] || 'viewer'
        };
        ...
        console.error('❌ Fehler beim Loggen der Twitch Nachricht:', error);
    }
}

// Twitch Command Handler
async function handleTwitchCommand(channel, tags, message) {
    try {
        const args = message.slice(twitchSettings.prefix.length).trim().split(/ +/);
        const commandName = args.shift().toLowerCase();

        if (!twitchCommands[commandName]) return;

        const command = twitchCommands[commandName];
        
        // Cooldown Check (vereinfacht)
        const now = Date.now();
        const cooldownKey = `${tags.username}_${commandName}`;
        
        // Response senden
        let response = command.response;
        response = response.replace(/{user}/g, tags['display-name'] || tags.username);
        response = response.replace(/{uptime}/g, twitchBotStatus.uptime);
        response = response.replace(/{game}/g, 'Aktuelles Game'); // TODO: Twitch API für Game

        await twitchBot.say(channel, response);

        // Command Stats speichern
        await saveTwitchCommandStats(channel, commandName, tags.username);

    } catch (error) {
        console.error('❌ Fehler beim Verarbeiten des Twitch Commands:', error);
    }
}

// Moderation Check
function shouldModerateMessage(message, tags) {
    if (!twitchSettings.moderation.antiSpam) return false;

    const { maxCaps, maxLength, bannedWords } = twitchSettings.moderation;

    // Caps Check
    const capsRatio = (message.match(/[A-Z]/g) || []).length / message.length;
    if (capsRatio > maxCaps / 100) return true;

    // Length Check
    if (message.length > maxLength) return true;

    // Banned Words Check
    const lowerMessage = message.toLowerCase();
    if (bannedWords.some(word => lowerMessage.includes(word))) return true;

    return false;
}

// Twitch Moderation Handler
async function handleTwitchModeration(channel, tags, message) {
    try {
        // Timeout User (30 Sekunden)
        await twitchBot.timeout(channel, tags.username, 30, 'Auto-Moderation');
        
        // Log Moderation
        await logTwitchModeration(channel, tags, 'timeout', 'Auto-Moderation');
        
        console.log(`🛡️ User ${tags.username} in ${channel} für 30s getimed (Auto-Mod)`);

    } catch (error) {
        console.error('❌ Fehler bei Twitch Moderation:', error);
    }
}

// Auto Responses
function handleAutoResponses(channel, tags, message) {
    // Implementiere Auto-Responses basierend auf Keywords
    const lowerMessage = message.toLowerCase();
    
    if (lowerMessage.includes('discord')) {
        setTimeout(async () => {
            await twitchBot.say(channel, 'Join unserem Discord Server! discord.gg/yourinvite');
        }, 1000);
    }
}

// Uptime Update
function updateTwitchUptime() {
    if (!twitchBotStatus.startTime) return;
    
    const now = new Date();
    const uptime = now - twitchBotStatus.startTime;
    const hours = Math.floor(uptime / (1000 * 60 * 60));
    const minutes = Math.floor((uptime % (1000 * 60 * 60)) / (1000 * 60));
    
    twitchBotStatus.uptime = `${hours}h ${minutes}m`;
}

// Status zu Supabase speichern
async function saveTwitchBotStatusToSupabase() {
    try {
        if (!supabase) return;

        await supabase
            .from('twitch_bot_status')
            .upsert({
                id: 1,
                is_running: twitchBotStatus.isRunning,
                connected_channels: twitchBotStatus.connectedChannels,
                total_messages: twitchBotStatus.totalMessages,
                uptime: twitchBotStatus.uptime,
                last_activity: twitchBotStatus.lastActivity,
                updated_at: new Date().toISOString()
            });

    } catch (error) {
        console.error('❌ Fehler beim Speichern des Twitch Bot Status:', error);
    }
}
...
app.post('/api/twitch/channels', async (req, res) => {
    try {
        const { channelName } = req.body;
        
        if (!channelName) {
            return res.status(400).json({ success: false, error: 'Channel Name erforderlich' });
        }

        // Channel zu Supabase hinzufügen
        if (supabase) {
            const { data, error } = await supabase
                .from('twitch_channels')
                .insert({
                    channel_name: channelName.toLowerCase().replace('#', ''),
                    active: true,
            ...
            twitchChannels.push(data);
        }

        // Bot zu neuem Channel hinzufügen
        if (twitchBot && twitchBotStatus.isRunning) {
            await twitchBot.join(channelName);
            twitchBotStatus.connectedChannels.push(channelName);
        }

        res.json({ success: true, channel: channelName });

    } catch (error) {
        console.error('❌ Fehler beim Hinzufügen des Twitch Channels:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});

app.delete('/api/twitch/channels/:channelName', async (req, res) => {
    try {
        const { channelName } = req.params;

        // Channel aus Supabase entfernen
        if (supabase) {
            await supabase
                .from('twitch_channels')
                .delete()
                .eq('channel_name', channelName.toLowerCase().replace('#', ''));
        }

        // Bot von Channel entfernen
        if (twitchBot && twitchBotStatus.isRunning) {
            await twitchBot.part(channelName);
            twitchBotStatus.connectedChannels = twitchBotStatus.connectedChannels.filter(ch => ch !== channelName);
        }

        // Lokale Liste updaten
        twitchChannels = twitchChannels.filter(ch => ch.channel_name !== channelName.toLowerCase().replace('#', ''));

        res.json({ success: true });

    } catch (error) {
        console.error('❌ Fehler beim Entfernen des Twitch Channels:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});
...
app.get('/api/twitch/stats', async (req, res) => {
    try {
        if (!supabase) {
            return res.json({ success: false, error: 'Supabase nicht verfügbar' });
        }
        ...
        const stats = {
            totalChannels: twitchChannels.length,
            dailyMessages: chatStats?.length || 0,
            uniqueUsers: new Set(chatStats?.map(log => log.username) || []).size,
            isRunning: twitchBotStatus.isRunning
        };

        res.json({ success: true, stats });

    } catch (error) {
        console.error('❌ Fehler beim Laden der Twitch Stats:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});

// Twitch Bot automatisch starten
setTimeout(() => {
    initializeTwitchBot();
}, 3000); // 3 Sekunden nach Discord Bot Start

// Fehlende Twitch Bot Funktionen
async function saveTwitchCommandStats(channel, command, username) {
    try {
        if (!supabase) return;

        await supabase
            .from('twitch_command_stats')
            .insert({
                channel: channel.replace('#', ''),
                command: command,
                username: username,
        ...
        console.error('❌ Fehler beim Speichern der Command Stats:', error);
    }
}

async function logTwitchModeration(channel, tags, action, reason) {
    try {
        if (!supabase) return;

        await supabase
            .from('twitch_moderation_logs')
            .insert({
                channel: channel.replace('#', ''),
                username: tags.username,
                user_id: tags['user-id'],
                ...
                reason: reason,
                timestamp: new Date().toISOString(),
                moderator: 'AgentBeeBot'
            });

    } catch (error) {
        console.error('❌ Fehler beim Loggen der Moderation:', error);
    }
}

// Uptime Timer
setInterval(() => {
    if (twitchBotStatus.isRunning) {
        updateTwitchUptime();
        saveTwitchBotStatusToSupabase();
    }
}, 60000); // Jede Minute

// ==================== END TWITCH BOT INTEGRATION ====================
...
app.listen(API_PORT, HOST, () => {
    ...
    console.log('   GET  /api/twitch/status');
    console.log('   POST /api/twitch/channels');
    console.log('   DELETE /api/twitch/channels/:channelName');
    console.log('   GET  /api/twitch/stats');
